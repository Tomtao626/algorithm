...
public class CircleLinkedList<E> extends AbstractList<E> {
	...
	public void add(int index, E element) {
		...
		if (index == size) {
			Node<E> oldLast = last;
			last = new Node<>(oldLast, element, first); // 指向之前最后一个元素的last
			if (oldLast == null) {
				// 链表添加的第一个元素
				first = last;
				first.next = first;
			...
			next.prev = node;
			prev.next = node;
			if (index == 0) { // index = 0 开头 // (next = first)
				first = node;
			}
		}
		size++;
	...
	private E remove(Node<E> node) {
		if (size == 1) {
			...
			if (node == first) { // index = 0 从开头删除 删除第一个
				first = next; // 第零个结点的下一个 == node.next
			}
			if (node == last) { // index = size - 1 从末尾删除 删除最后一个
